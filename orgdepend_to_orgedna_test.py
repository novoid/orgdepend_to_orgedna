#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# Time-stamp: <2020-09-19 23:35:30 vk>
...
class TestMethods(unittest.TestCase):
    ...
    def NOtest_trigger_regex_matches(self):

        # FIXXME: do not only check match/non-match but also number of matching elements:
        
        # normal org-depend lines:
        self.assertTrue(orgdepend_to_orgedna.get_trigger_matches(':TRIGGER:    2016-11-04-some-title(DONE)'))
        self.assertTrue(orgdepend_to_orgedna.get_trigger_matches(':TRIGGER: 2016-11-04-some-title(DONE)'))
        self.assertTrue(orgdepend_to_orgedna.get_trigger_matches(':TRIGGER: 2016-11-04-some-title(DONE)  2020-09-19-foo(STARTED)'))
    ...
    def NOtest_simple_trigger_line(self):

        self.assertEqual(orgdepend_to_orgedna.convert_trigger_line(':TRIGGER:    2016-11-04-some-title(DONE)'),
                         ':TRIGGER: ids(2016-11-04-some-title) todo!(DONE)'
                         )

    def NOtest_trigger_line_with_elisp_snippet(self):

        self.assertEqual(orgdepend_to_orgedna.convert_trigger_line(':TRIGGER: prefix-`(my-capture-insert \'my-1)`-notes(STARTED)    prefix2-`(my-capture-insert \'my-foo)`-bar(NEXT)'),
